...
ROLLBACK;
--> COMMIT 되지 않은 INSERT,UPDATE,DELETE(DML)을 취소
...
WHERE 
	EMP_NAME IN ('노옹철', '정중하', '전형돈', '하동운'); -- 4행 수정
...
FROM
	EMPLOYEE2
WHERE 
	EMP_NAME IN ('노옹철', '정중하', '전형돈', '하동운');

ROLLBACK;

-- EMP_SALARY테이블에서 아시아지역에 근무하는 직원의 보너스를
-- 0.3으로 변경


-- 1) 아시아 지역에 근무하는 직원
SELECT 
	E.EMP_ID, E.EMP_NAME, E.BONUS, L.LOCAL_NAME 
FROM
	EMPLOYEE2 E
JOIN
	DEPARTMENT2 D ON (E.DEPT_CODE = D.DEPT_ID)
JOIN 
...
/* EMP_M01을 기준으로하여 EMP_M02를 병합
 * 1) 같음을 판별한 컬럼 기준 작성
 * 		-> ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID)
 * 
 ...
 * 		-> INSERT 수행
 * */
MERGE INTO EMP_M01 USING EMP_M02 
ON(EMP_M01.EMP_ID = EMP_M02.EMP_ID)
WHEN MATCHED THEN
UPDATE SET
 ...
 * - 자주 조회되는 데이터 전용 테이블과
 * 	 전체 데이터 저장 테이블간의 병합
 * ex.서점에서 유명한 책 검색시 인기 있는책 
 * 	  테이블 쓰다가 나중에 전체 테이블에 병합
 * */
ROLLBACK;
