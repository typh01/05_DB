...
/* 함수 : 컬럼갑 | 지정된 값을 읽어 연산한 결과를 반환하는 것
 * 
 *
 * 1. 단일행 함수 (N -> N) 
...
SELECT 
	CEIL((SYSDATE - TO_DATE('2001.03.20', 'YYYY.MM.DD')) / 365 + 1)
	AS "한국식 나이",
	
	FLOOR(MONTHS_BETWEEN(SYSDATE, TO_DATE('2001.10.15', 'YYYY.MM.DD')) / 12) 
	AS "만 나이"
FROM DUAL;
/********************************************/
-- 정확한 날짜 구할 때는 MONTH_BETWEEN이 좋다!
--> 내부적으로 윤달 계산이 포함됨 
/*******************************************/
...
-- 오늘 날짜 변환
-- '2025년02월20일 (목) 오후 04시 30분 20초' 변환
SELECT 
...
-- RPAD(문자열, 길이, 바꿀 문자)
--> 문자열 전체에서 오른쪽을 지정된 길이 만큼 바꿀 문자열 리턴
...
SELECT 
	MAX(SALARY), MIN(SALARY) -- 4900000 3800000
FROM 
	EMPLOYEE
WHERE 
	DEPT_CODE = 'D6';

-- 모든 사원 중 이름순으로 오름차순 정렬 했을 때
...
/* 방법 2 - COUNT(컬럼명) 이용 */
SELECT 
	COUNT(PHONE) -- PHONE 컬럼의 값이 NULL인 경우 제외하고 카운트
FROM 
	EMPLOYEE; 
