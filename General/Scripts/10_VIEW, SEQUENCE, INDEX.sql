 ...
 * 
 *  2) FORCE | NOFORCE 옵션 : 
 *    FORCE : 서브쿼리에 사용된 테이블이 존재하지 않아도 뷰 생성 - DB 마이그레이션 때 사용 - 보통 안씀
...
CREATE VIEW V_EMP
AS 
SELECT E.EMP_ID AS 사번, E.EMP_NAME AS 이름, NVL((D.DEPT_TITLE), '부서없음') AS 부서명, J.JOB_NAME AS 직급명
FROM EMPLOYEE E 
...
/* VIEW 를 이용한 DML 사용시 문제점 */

-- DEPARTMENT 테이블을 복사한 DEPT_COPY2 테이블 생성
 CREATE TABLE DEPT_COPY2
 AS SELECT * FROM DEPARTMENT;
--> DEPT_COPY2 테이블의 제약조건 확인
-- DEPT_ID, LOCATION_ID : NOT NULL
-- DEPT_TITL : NULL 


-- 값이 무조건 들어가야하는 컬럼만 이용해서 VIEW 생성
...
AS
SELECT
	DEPT_ID, DEPT_TITLE -- LOCATION_ID 제외
FROM
	DEPT_COPY2;
...
INSERT INTO V_DCOPY2
VALUES('A1', '마케팅1팀');
-- ("KH12_PTH"."DEPT_COPY2"."LOCATION_ID") 안에 삽입할 수 없습니다

-- V_DCOPY2를 이용하면 LOCATION_ID 컬럼에 
-- INSERT 할 수 없기 때문에 자동으로 NULL이 삽입됨
--> LOCATION_ID는 NOT NULL 제약조건이 설정되어 있기 때문에
--  NULL 삽입 불가!

--> 테이블, VIEW 구조를 파악하지 못하고 있으면 
-- VIEW를 이용한 DML 수행 시 잦은 오류가 발생!!

/* VIEW READ ONLY OPTION 
 * -> VIEW는 ALTER 불가 -> CREATE OR REPLACE
 * */
CREATE OR REPLACE VIEW V_DCOPY2
AS
SELECT
...
NOMINVALUE			 -- 최소값 없음(CYCLE과 관련된 속성)
NOCYCLE 				 -- 순환 X
...
-- == NEXTVAL가 한 번도 호출 된 적이 없다

SELECT SEQ_TEST_NO.NEXTVAL FROM DUAL; -- 100
SELECT SEQ_TEST_NO.CURRVAL FROM DUAL; -- 100(오류 없음)
...
WHERE 
	TEST_NAME = '짱구'; -- 140 -> 145 -> 150
-- 150 이상으로 증가 불가(MAXVALUE)
	-- 변경 후 ~200 까지
...
/* 인덱스를 활용해서 검색하는 방법 
 * SELECT문의 WHERE 절에 INDEX가 설정된 을 언급하면 된다
 * */
...
SELECT * FROM TB_IDX_TEST 
WHERE TEST_ID = 'TEST500000'; -- 0.04s

/* 인덱스를 이용해서 조회하기
 * - TEST_ID 컬럼 값이 500000 인 행 조회
 * */
SELECT * FROM TB_IDX_TEST 
WHERE TEST_NO = 500000; -- 0.03s
