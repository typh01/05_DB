...
SELECT 
	S.STUDENT_NO 학번, S.STUDENT_NAME 이름, D.DEPARTMENT_NAME "학과 이름", TRUNC(AVG(G.POINT),8) 평점
FROM 
	TB_STUDENT S
JOIN 
	TB_DEPARTMENT D ON(S.DEPARTMENT_NO = D.DEPARTMENT_NO)
...
SELECT 
	CLASS_NO, CLASS_NAME, TRUNC(AVG(POINT),2) 평점
FROM 
	TB_CLASS 
JOIN 
...
FROM(
	SELECT 
		STUDENT_NO, STUDENT_NAME, AVG(POINT) 평점
  FROM 
  	TB_GRADE
  JOIN 
  	TB_STUDENT USING(STUDENT_NO)
  WHERE 
  	DEPARTMENT_NO  = (
  		SELECT DEPARTMENT_NO
      FROM TB_DEPARTMENT
...
SELECT 
	DEPARTMENT_NAME "계열 학과명", ROUND(AVG(POINT),1) 전공평점
FROM 
	TB_DEPARTMENT 
JOIN 
	TB_CLASS USING(DEPARTMENT_NO)
JOIN 
	TB_GRADE USING(CLASS_NO)
WHERE 
	CATEGORY = (
