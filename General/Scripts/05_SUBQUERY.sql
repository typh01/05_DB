...
SELECT
	E.EMP_NAME, J.JOB_NAME, D.DEPT_TITLE, E.SALARY
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
LEFT OUTER JOIN -- 왼쪽 자료들의 NULL 도 포함
...
SELECT 
	E.EMP_ID, E.EMP_NAME, J.JOB_NAME ,E.DEPT_CODE, E.SALARY, E.HIRE_DATE 
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
SELECT 
	E.EMP_ID, E.EMP_NAME, D.DEPT_TITLE, J.JOB_NAME, E.SALARY 
FROM
	EMPLOYEE E
JOIN
...
-- WHERE
	-- FROM 절 테이블에서 조회를 원하는 행에 조건
	
-- HAVING
	-- GROUP BY절에 의해 만들어지 그룹 중 조회를 원하는 그룹에 대한 조건
...
SELECT 
	NVL(D.DEPT_TITLE, '없음') AS DEPT_TITLE , COUNT(*) 
FROM
	EMPLOYEE E
LEFT JOIN
	DEPARTMENT D ON (D.DEPT_ID = E.DEPT_CODE)
...
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
WHERE
	E.EMP_ID NOT IN ( -- 사수 역할을 갖지 않는 사원만 조회
		SELECT 
			DISTINCT MANAGER_ID
		FROM
			EMPLOYEE 
...
SELECT 
	E.EMP_ID, EMP_NAME, J.JOB_NAME, E.SALARY 
FROM
	EMPLOYEE E
JOIN
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
WHERE 
	J.JOB_NAME = '대리'
AND
	E.SALARY > ( -- 과장 최소 급여(320만)보다 많이 받는 대리 조회
		SELECT 
			MIN(E.SALARY)
		FROM
			EMPLOYEE E
...
WHERE 
	J.JOB_NAME = '대리'
AND
	E.SALARY > ANY ( -- 한개라도 크면 -> 가장 작은 것 보다 크면!
		SELECT -- 476만, 320만, 350만
			E.SALARY
		FROM
			EMPLOYEE E
		JOIN
...
WHERE 
	J.JOB_NAME ='과장'
AND 
	E.SALARY > ALL( -- 제일 큰것보다 큰가?
		SELECT
			E.SALARY -- 380, 348, 349, 255
		FROM
			EMPLOYEE E
		JOIN
			JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
WHERE DEPT_CODE IN (
	...
	SELECT DEPT_ID -- 2) 서브 쿼리
	FROM DEPARTMENT 
	WHERE 
		LOCATION_ID = (
		
			SELECT LOCAL_CODE -- 1) 서브 쿼리
			FROM LOCATION 
...
SELECT 
	E.EMP_ID, E.EMP_NAME, E.DEPT_CODE, J.JOB_CODE, D.DEPT_TITLE, J.JOB_NAME
FROM
	EMPLOYEE E 
JOIN 
	JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
FROM
	EMPLOYEE
WHERE 
	SALARY IN (3000000, 7000000);


-- 2) 직급별 평균 급여
SELECT 
	JOB_CODE, TRUNC(AVG(SALARY), -4)  
FROM 
	EMPLOYEE 
GROUP BY
	JOB_CODE;
...
WHERE 
	(JOB_CODE, SALARY) IN (
		SELECT JOB_CODE, TRUNC(AVG(SALARY), -6)  
		FROM EMPLOYEE 
		GROUP BY JOB_CODE
	);                  
...
-- 기존하고 순서(서브 -> 메인)가 반대 (메인 -> 서브)

-- 상관쿼리는 먼저 메인쿼리 한 행을 조회하고
-- 해당 행이 서브쿼리의 조건을 충족하는지 확인하여 SELECT를 진행함

	-- ex. 안읽은 메시지 개수, 참여 인원 수, 댓글 수, 게시글 1 + N개의 댓글, 좋아요

/* EXISTS(서브쿼리)
 * - 서브쿼리 조회 결과가 
 * 	 1행 이상 존재하면 TRUE -> MAIN QUERY 결과 포함 O
 * 	 0행이면 FALSE 반환			-> MAIN QUERY 결과 포함 X
 *  */
...
WHERE
	MAIN.HIRE_DATE = (
		...
		FROM 
			EMPLOYEE SUB
		WHERE 
			NVL(SUB.DEPT_CODE, '소속 없음') 
			= NVL(MAIN.DEPT_CODE , '소속 없음') -- NULL 등호 비교 불가
		AND 
			ENT_YN != 'Y' 	
	)
ORDER BY 
...
ORDER BY 
	"급여 평균" DESC;
			...
			-- GROUP BY에 SELCT절에 원하는 값이 존재해야함

-- 2) 1번 SELECT문을 인라인뷰로 사용하여
-- 상위 3개 부서만 조회
SELECT 
	ROWNUM, 
	"부서 코드", "부서명", "급여 평균"
...
-- 동일한 인원 수 만큼 건너뛰고 순위 계산
-- EX) 공동 1위가 2명이면 다음 순위는 2위가 아니라 3위

-- OVER() 괄호 내에 정렬 기준을 작성
--> 정렬이 되면서 순위 지정됨
---> INLINE VIEW, ROWNUM 없이도 순위 지정 가능
SELECT 
...
/* [ROWNUM 사용 시 주의사항!]
 * 
 * ROWNUM 이 WHERE절에 사용되는 경우
 * 항상 1부터 연속적인 범위가 지정되어야 한다!!!
 * -> ROWNUM은 RESULT SET 완성 후 부여되는
 * 		가상의 컬럼으로써
 * 		정해진 규칙(1부터 1씩 증가)을 만족하지 못하면 
 * 		사용 불가
 * */
...
-- 사번, 사원명, 전화번호, 고용일, 부서명
SELECT 
	E.EMP_ID, E.EMP_NAME, E.PHONE, E.HIRE_DATE, D.DEPT_TITLE
FROM
	EMPLOYEE E
JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
-- 사번, 사원명, 전화번호, 급여, 직급명을 조회하시오.
	SELECT 
		E.EMP_ID, E.EMP_NAME, E.PHONE, E.SALARY, J.JOB_NAME
	FROM
		EMPLOYEE E
	LEFT JOIN
		JOB J ON (E.JOB_CODE = J.JOB_CODE)
...
-- 사번, 이름, 부서코드, 사수번호, 주민번호, 고용일
SELECT 
	EMP_ID, EMP_NAME, DEPT_CODE, EMP_NO ,MANAGER_ID, HIRE_DATE 
FROM
	EMPLOYEE E
WHERE 
	(DEPT_CODE , JOB_CODE) = (
...
-- 단, 퇴사한 직원은 제외하고 조회..
SELECT 
	E.EMP_ID, E.EMP_NAME, NVL(D.DEPT_TITLE, '소속없음') DEPT_TITLE , J.JOB_NAME, E.HIRE_DATE 
FROM 
	EMPLOYEE E
LEFT JOIN
	DEPARTMENT D ON (E.DEPT_CODE = D.DEPT_ID)
...
WHERE 
	HIRE_DATE = (
		...
		FROM EMPLOYEE SUB
		WHERE ENT_YN != 'Y'
		AND NVL(SUB.DEPT_CODE, '없음') = NVL(E.DEPT_CODE, '없음') 
	)
ORDER BY
	E.HIRE_DATE DESC;
...
SELECT 
	...
	FLOOR(
		MONTHS_BETWEEN(
			SYSDATE, 
			TO_DATE(19||SUBSTR(EMP_NO,1,6), 'YYMMDD')
		) / 12
	) AS "만 나이",
	TO_CHAR(
		SALARY * (1 + NVL(BONUS, 0)) * 12, 'L999,999,999'
		) AS "연봉"
FROM
	EMPLOYEE E
JOIN
...
WHERE 
	TO_NUMBER(SUBSTR(E.EMP_NO,1,6)) = (
		SELECT 
			MAX(TO_NUMBER(SUBSTR(SUB.EMP_NO,1,6)))
		FROM
			EMPLOYEE SUB
		WHERE 
			SUB.JOB_CODE = E.JOB_CODE 
		)
ORDER BY 
	"만 나이" DESC; 
