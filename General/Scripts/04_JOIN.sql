...
-- 21행 조회
-- 왜? 23행이 아니라 21행??
-- EMPLOYEE 에 DEPT_CODE 값이 NULL인 행이 2개 있지만
-- DEPARTMENT의 DEPT_ID는 NULL인 행이 없기 때운에 연결 X
...
--	SAL_GRADE USING(SAL_LEVEL); 등가 조인
	
		-- 신용카드 등급, 요금제 회원 등급에 사용
...
-- 1) ON 이용 방법 -> JOIN에 사용된 컬럼이 별도로(2개) 존재
SELECT 
...
-- 2.1 이름에 '형'자가 들어가는 직원들의 사번, 사원명, 직급명을 조회하시오.
SELECT 
...
--	D.DEPT_TITLE LIKE '해외영업1부'
--OR
--	D.DEPT_TITLE LIKE '해외영업2부'
ORDER BY 
	E.EMP_ID ASC;
...
OR 
	N.NATIONAL_CODE LIKE 'JP';


-- 8. 같은 부서에 근무하는 직원들의 사원명, 부서코드, 동료이름을 조회하시오.
-- SELF JOIN 사용
SELECT
	E1.EMP_NAME, E1.DEPT_CODE, E2.EMP_NAME 
FROM
	EMPLOYEE E1
INNER JOIN
